...
impl LevelGrid {
    ...
    pub fn effective_tile(&self, x: i32, y: i32, holes: &HoleMap) -> Tile {
        ...
        if base == Tile::Brick {
            if let Some(phase) = holes.get(x, y) {
                return match phase {
                    HolePhase::Open => Tile::Empty,
                    HolePhase::Closing => Tile::Brick, // closing = non-enterable
                };
            }
        }
        base
...
impl HoleMap {
    ...
    pub fn cells(&self) -> HashSet<(i32, i32)> {
        self.holes.keys().copied().collect()
    }
}
...
=====",
    ...
    fn out_of_bounds_is_solid() {
        ...
        assert!(g.is_solid(0, -1));
        assert!(g.is_solid(5, 0));
        assert!(g.is_solid(0, 4));
    }
...
===",
        ...
        let h = empty_holes();
        // Before reveal: (1,2) is HiddenLadder, can't climb up from (1,1) ladder to it
        // Actually can_climb_up checks if current tile is ladder, and above is not solid
        // HiddenLadder is not solid, so climbing up from ladder below works
        // But the hidden ladder itself can't be climbed from
        assert!(!parsed.grid.can_climb_up(1, 2, &h)); // can't climb up FROM hidden ladder

        parsed.grid.reveal_hidden_ladders();
        assert_eq!(parsed.grid.get(1, 2), Tile::Ladder);
        assert!(parsed.grid.can_climb_up(1, 2, &h)); // now it's a real ladder
    }
}
