...
- `src/player.rs`: player input, gravity, movement, tile collision, left-camera clamp, and pit death detection
- `src/death.rs`: death animation flow for `PlayState::Dying`, including respawn or game-over transition
- `src/level_complete.rs`: flagpole collision handling and the scripted flagpole-to-castle level-complete sequence
- `src/camera.rs`: camera setup, reset, and follow logic
- `src/ui.rs`: start screen, HUD, countdown timer, pause overlay, game-over UI flow, and score popup helpers
- `src/enemy/mod.rs`: shared enemy plugin wiring, activation, shared enemy physics, score popups, and common damage helper
- `src/enemy/goomba.rs`: Goomba plugin, spawner registration, and Mario-Goomba stomp/damage behavior
- `src/enemy/koopa.rs`: Koopa plugin, spawner registration, shell assets, and Mario-Koopa/Mario-shell/shell-enemy interactions
...
- `src/resources.rs`: `GameData`, `GameTimer`, score/coin messages, `SpawnPoint`, death-animation resources, pending block-hit state, and level-complete animation state
...
- Startup initializes `GameData`, `GameTimer`, `SpawnPoint`, `ScoreEvent`/`CoinEvent` messages, shared `GameAssets`, the custom `LevelAssetLoader`, and a `SpawnerRegistry`.
...
- `assets/levels/1-1.level.ron` and `assets/levels/test.level.ron` exist, while `src/level.rs` still keeps hardcoded generators plus a test that can regenerate those RON files.
- Entering `AppState::Playing` builds the `LevelGrid`, applies level metadata for timer, world name, and background color, records the `SpawnPoint`, spawns the world from the loaded grid through registry-driven tile/entity spawners, and creates the HUD.
- `LevelData` also supports `gravity_multiplier` metadata in the RON format, but the current runtime only applies timer, world name, and background color.
...
- Colored primitive meshes are spawned from shared asset handles for world tiles, Mario, power-ups, fireballs, flagpole/castle pieces, brick particles, and score popups, while enemy modules own their own visuals and spawner registration.
...
- The scripted level-complete sequence lives in `src/level_complete.rs`.
...
- Preserve the existing split between `main.rs`, `assets.rs`, `components.rs`, `constants.rs`, `collision.rs`, `level.rs`, `player.rs`, `death.rs`, `level_complete.rs`, `camera.rs`, `block.rs`, `powerup.rs`, `ui.rs`, `resources.rs`, `states.rs`, and `src/enemy/`.
...
- Current visuals still use Bevy 2D primitives (`Rectangle`, `Mesh2d`, `MeshMaterial2d`) rather than sprite assets, but those shared handles are centralized in `src/assets.rs` and the enemy asset resources in `src/enemy/`.
...
- Level asset loading, RON format, grid helpers, and spawn registry: `src/level.rs`
- Player movement and pit-death detection: `src/player.rs`
- Death animation and respawn/game-over flow: `src/death.rs`
- Flagpole detection and level-complete sequence: `src/level_complete.rs`
...
- UI/state transitions and HUD: `src/ui.rs`
