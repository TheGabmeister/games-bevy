...
- **`schedule.rs`** ✅ — `SchedulePlugin`; defines the `Physics` `SystemSet` and chains its `FixedUpdate` phase order once (see **Physics & System Ordering**)
...
- **`collision.rs`** ✅ — `CollisionPlugin`; ball↔wall/paddle/brick reflection (silver durability, indestructible gold, Catch sticks instead of bouncing), multi-ball-aware bottom loss, triggers `BallLost`, emits `BounceSound`. Also exports the shared `ball_box_bounce` (AABB reflect-off-least-penetrated-face) and `damage_brick` (durability/score/despawn) helpers, reused by laser↔brick (`powerups.rs`) and ball↔enemy (`enemy.rs`) collision
...
Gameplay physics runs in **`FixedUpdate`** for determinism. Cross-plugin ordering is expressed with the `Physics` **`SystemSet`** (`schedule.rs`), chained once in `SchedulePlugin`: `PaddleInput → BallFollow → Movement → Collision`. Each gameplay plugin tags its `FixedUpdate` systems with `.in_set(Physics::…)` rather than `.after(other_plugin::system)`; the only remaining `.after()` calls are intra-set dependencies (e.g. `ball_brick_collision.after(ball_collision)`, `accelerate_ball.after(ball_movement)`). Systems keep their own `run_if` state gating (paddle/ball-follow run through all of `Playing`; movement/collision only while `Running`) — the sets order, they don't gate. One-shot input reactions (e.g. `ball_launch`) run in `Update`. Because ordering no longer reaches across modules by function reference, those systems need not be `pub`.
