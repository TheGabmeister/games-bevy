...
- **`resources.rs`** — `GameData` (score/lives/level/bays), `FrogTimer`, `LevelState` (speed multiplier, celebration timer), plus `Message` types (`FrogDeath`, `FrogBayFilled`, `SpawnDeathFlash`, `SpawnScorePopup`)
...
- **`effects.rs`** — Death flash (expanding fading circle) and score popup (rising fading text) driven by `SpawnDeathFlash`/`SpawnScorePopup` messages
...
- The old `EventWriter<T>`/`EventReader<T>` API has been replaced by **`MessageWriter<T>`/`MessageReader<T>`**. Derive with `#[derive(Message)]` and register with `app.add_message::<T>()`. Use `.write()` to send and `.read()` to iterate received messages.
